...
public class HttpUploadServerHandler extends SimpleChannelInboundHandler<HttpObject>{
	...
	protected void messageReceived(ChannelHandlerContext ctx, HttpObject msg) throws Exception {
		if(msg instanceof HttpRequest){
			...
			QueryStringDecoder decoderQurey = new QueryStringDecoder(request.uri());
			Map<String, List<String>> uriAttributes = decoderQurey.parameters();
			...
			//如果是get请求  不再创建一个HttpPostRequestDecoder
	...
	private void writeResponse(Channel channel){
		//将response 转化为一个ChannelBuffer
		ByteBuf buf = copiedBuffer(responseContent.toString(),CharsetUtil.UTF_8);
		responseContent.setLength(0);

		//是否关闭这个连接Connection
		boolean close = request.headers().contains(HttpHeaderNames.CONNECTION,
				HttpHeaderValues.CLOSE,true)||request.protocolVersion().equals(HttpVersion.HTTP_1_1)
		...
		if(!cookies.isEmpty()){
			...
			for (Cookie cookie : cookies) {
				response.headers().add(HttpHeaderNames.SET_COOKIE,ServerCookieEncoder.encode(cookies));
			}
		}
